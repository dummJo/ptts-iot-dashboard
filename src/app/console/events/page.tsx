  ...
  return (
    <div className="flex min-h-screen relative overflow-hidden" style={{ background:"var(--bg)" }}>
      ...
      <main className="flex-1 overflow-auto flex flex-col relative z-10">
        <TopBar title="Event Center" onRefresh={handleRefresh} refreshing={refreshing} connected={dashboardData?.system?.connected} pollInterval={pollInterval} onPollChange={setPollInterval} />
        ...
        <div className="flex-1 p-6 space-y-4">
          <div className="flex gap-2">
            <button 
              ...
              className="text-xs px-3 py-1.5 font-bold tracking-widest transition-all shadow-sm active:scale-95 disabled:opacity-40" 
              style={{ background:"rgba(0, 230, 118, 0.1)", color:"var(--online)", border:"1px solid #00e67640" }}>
              {refreshing ? "PROCESSING..." : "✓ ACKNOWLEDGE ALL"}
            </button>
            <button 
              onClick={handleExportLog}
              disabled={dashboardData.recentAlerts.length === 0}
              className="text-xs px-3 py-1.5 font-bold tracking-widest transition-all shadow-sm active:scale-95 disabled:opacity-40 hover:bg-surface-3" 
          ...
          <div className="scada-card p-4 min-h-[300px]">
             ...
             <div className="flex items-center justify-center p-8 bg-[var(--surface-inset)] border border-dashed mt-4" style={{ borderColor:"var(--border-dim)" }}>
